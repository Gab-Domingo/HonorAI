...
# Page configuration
st.set_page_config(page_title="Legal Document Analyzer",
                   page_icon="⚖️",
                   layout="wide")
...
tab1, tab2, tab3 = st.tabs(
    ["📄 Document Analysis", "💬 Legal Assistant", "📚 Saved Documents"])
    ...
    if uploaded_file is not None:
        with st.spinner("Processing document..."):
            try:
                ...
                st.info(
                    f"Document text extracted successfully (length: {len(document_text)} characters)"
                )
            except Exception as e:
            ...
            if st.button("Analyze Document"):
                try:
                    with st.spinner("Analyzing document with AI..."):
                        ...
                        # Save to database
                        document_id = save_document(uploaded_file.name,
                                                    document_text,
                                                    analysis_results)
                        if document_id:
                            ...
                            st.success(
                                f"Document saved to database with ID: {document_id}"
                            )
                        else:
                    ...
                    for i, (entity_type,
                            entity_values) in enumerate(entity_types.items()):
                        col_idx = i % 3
                    ...
                    highlighted_text = utils.highlight_entities_in_text(
...
with tab2:
    ...
    if st.session_state.document_text is None:
        st.warning(
            "Please upload and analyze a document first to use the chatbot.")
    else:
            ...
            with st.spinner("Generating response..."):
                if st.session_state.document_embeddings:
                    ...
                    context = perform_document_search(
                        user_question, st.session_state.document_embeddings,
                    ...
                    response = get_chatbot_response(
                        user_question, st.session_state.document_text,
                        st.session_state.document_analysis)
...
with tab3:
    ...
    try:
        ...
        if not documents:
            st.info(
                "No documents have been saved yet. Upload and analyze a document to save it."
            )
        else:
            ...
            for doc in documents:
                with st.expander(
                        f"{doc['filename']} - {doc['document_type'] or 'Unknown Type'} - {doc['upload_date'].strftime('%Y-%m-%d %H:%M')}"
                ):
                    st.write(
                    ...
                    # Load button
                    if st.button(f"Load Document #{doc['id']}",
                                 key=f"load_{doc['id']}"):
                        # Load document from database
                        full_doc = get_document_by_id(doc['id'])
                        if full_doc:
                            # Update session state
                            st.session_state.document_text = full_doc[
                                'document_text']
                            st.session_state.current_document_id = full_doc[
                                'id']
                            ...
                            st.success(
                                f"Document #{doc['id']} loaded successfully!")
                            st.rerun()

                    # Delete button
                    if st.button(f"Delete Document #{doc['id']}",
                                 key=f"delete_{doc['id']}"):
                        if delete_document(doc['id']):
                            st.success(
                                f"Document #{doc['id']} deleted successfully!")

                            # Clear session state if the current document is deleted
                            if st.session_state.current_document_id == doc[
                                    'id']:
                                st.session_state.document_text = None
                                st.session_state.document_analysis = None
                                st.session_state.entities = None
